...
import {loadPhoto, loadPhotoTags} from "../../lib/loadPhoto";
import {getNoun} from "../api/noun";

const cx = classNames.bind(styles);
const cxPhoto = classNames.bind(stylesPhoto);

export async function getStaticProps() {
  const photoPosts = await loadPhoto();
        ...
        >
          <Link className="link" href={`/photo/${posts[0].id}`}>
            <picture className={cxPhoto('photo-article__img')}>
              <Image width={200} height={200} src={`https://rnmso.ru${posts[0].main_photo.image}`} alt="" />
            </picture>
